...
app.post("/getUserByName", async (req, res)=>{
    try {
        ...
        if(!userDetails){
            return res.status(404).send({message:"User details not found by its name"});
        }
        res.status(200).json({userDetails:userDetails});
    } catch (error) {
        console.log("Error in getting the user by its name", error);
        res.status(500).send({message:"Error in getting the user by its name"});
    }
})
